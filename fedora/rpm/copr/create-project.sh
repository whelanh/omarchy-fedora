...
DESCRIPTION="Omarchy Quattro first-party packages (aether, cliamp, elsewhen, herdr, hyprland-preview-share-picker, omacalc, omacut, omawrite, omasnap, owe, owe-lockfeed, tensaku, try, ttfx), source-built/repacked from fedora/rpm."
INSTRUCTIONS="Enabling the repo: sudo dnf copr enable ${COPR}; then: sudo dnf install aether cliamp elsewhen herdr hyprland-preview-share-picker omacalc omacut omawrite omasnap owe owe-lockfeed tensaku try ttfx"
