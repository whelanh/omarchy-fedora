...
This directory scaffolds Fedora RPM packaging for the "first-party"
packages that Omarchy ships but that are **not** available (or not maintained)
in Fedora official/COPR repos. They map to `source: copr` entries in
`fedora/mappings/packages.yaml` and are distributed via the
`whelanh/omarchy` COPR.

> **Status: 11 VERIFIED, 0 BLOCKED.** Every package builds cleanly in the
...
- **ttfx / hyprland-preview-share-picker** — Rust builds. COPR/mock build
  chroots are OFFLINE, so the crates.io dependencies are **vendored** into a
  `<name>-<version>-vendor.tar.xz` Source (generated by
  `fedora/rpm/vendor-rust.sh` during SRPM creation) and the spec builds with
  `cargo build --offline --locked`. The share-picker additionally swaps in the
  pinned `hyprland-protocols` submodule and needs `gtk4-layer-shell-devel` from
  the `nett00n/hyprland` COPR.
